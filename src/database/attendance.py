...
def employee_check_out(employee_id: int) -> bool:
    ...
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                "UPDATE attendance_records SET check_out = %s WHERE record_id = %s",
...
# --- Report helpers used by reports.py ---

def get_employee_details_by_date_range(employee_id: int, start_date: date, end_date: date) -> dict:
    """Compute total hours and absences for an employee between [start_date, end_date).
    Absences count weekdays with no attendance record, capped to today.
    """
    # Compute working days (Mon-Fri) between start and min(end-1, today)
    def count_weekdays(start: date, end_exclusive: date) -> int:
        end_inclusive = min(end_exclusive - timedelta(days=1), date.today())
    ...
    conn = get_db_connection()
    if not conn:
        return {"hours": 0.0, "absences": 0}
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                """
                SELECT 
                    ROUND(COALESCE(SUM(TIMESTAMPDIFF(MINUTE, check_in, IFNULL(check_out, NOW()))/60.0), 0), 2) AS hours,
                    COUNT(DISTINCT date) AS attended_days
                FROM attendance_records
            ...
            row = cursor.fetchone() or {"hours": 0.0, "attended_days": 0}
            working_days = count_weekdays(start_date, end_date)
            attended_days = int(row.get('attended_days', 0) or 0)
            absences = max(0, working_days - attended_days)
            return {"hours": float(row.get('hours', 0.0) or 0.0), "absences": absences}
    finally:
        conn.close()


def get_employee_monthly_attendance_details(employee_id: int, start_date: date, end_date: date) -> list[dict]:
    """Return daily attendance rows for weekdays within [start_date, end_date),
    synthesizing Absent rows for missing days. Fields: date, check_in, check_out, status, daily_hours.
    """
    conn = get_db_connection()
    if not conn:
        # Build synthetic with just dates
        return []
    try:
